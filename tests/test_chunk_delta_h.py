...
from cula.ops.cp.merge import merge_fwd  # noqa: E402
...
def test_cp_stage0_precompute_indices():
    ...
    (non_first, first_idx, last_idx, num_nf, seq_starts, seq_counts, init_off) = result
...
def test_cp_e2e_single_seq(T, H, use_gk):
    ...
    v_rel = (
        (v_cp.float() - v_base.float()).abs().max().item() / (v_base.float().abs().max().item() + 1e-8)
        if v_cp is not None
        else 0
    )
...
def test_cp_e2e_with_h0():
    ...
    h_rel = (h_cp.float() - h_base.float()).abs().max().item() / (h_base.float().abs().max().item() + 1e-8)
    ht_rel = (
        (ht_cp.float() - ht_base.float()).abs().max().item() / (ht_base.float().abs().max().item() + 1e-8)
        if ht_cp is not None
        else 0
    )
    assert h_rel < 0.05, f"h relative error too large: {h_rel}"
