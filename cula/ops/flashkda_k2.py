...
def k2_phaseB_kernel(
    ...
    sTmp = smem.allocate_tensor(cutlass.Float32, qk_layout, 128)  # noqa: F841
    sGt = smem.allocate_tensor(cutlass.Float32, cute.make_layout((D,)), 128)
    sBeta = smem.allocate_tensor(cutlass.BFloat16, cute.make_layout((CHUNK,)), 128)
...
# ===========================================================================
# Phase C: Phase B + swizzled sState SMEM layout to break the 32-way bank
# conflict pattern when threads read sState[kk, col_base+ee] (phase 1/4) and
# sState[tidx, cc] (phase 5). The conflict was the dominant SMEM stall source
# (43.8% of shared loads). Math is bit-identical to Phase B.
#
# State layout: K-major 8x64 atom with Swizzle<3,3,3> tiled to (D, D).
# This is the same atom CUTLASS uses for K_INTER_Atom<bf16> and is what the
# C++ K2 uses for state_acc. With this layout, sState reads from any 8 lanes
# in the same atom-row scatter across 8 different banks.
# ===========================================================================
def _make_state_smem_layout():
    # K-major 8x64 atom (rows ?? cols), swizzled
...
def k2_phaseC_kernel(
    ...
    qk_layout = cute.make_layout((CHUNK, D), stride=(D, 1))
    cc_layout = cute.make_layout((CHUNK, CHUNK), stride=(CHUNK, 1))
    state_layout = _make_state_smem_layout()  # swizzled
...
def run_k2_phaseC(
    ...
    smem_bytes = D * D * 2 + 5 * (CHUNK * D * 2) + (CHUNK * D * 2) + 2 * (CHUNK * CHUNK * 2) + (D * 4) + (CHUNK * 2) + 16 + 256
...
_compiled_cache_k2C: dict = {}


def launch_k2_phaseC(
    v: torch.Tensor,
    beta: torch.Tensor,
    ws_qd: torch.Tensor,
    ws_kd: torch.Tensor,
    ...
    """Run K2 Phase C (TMA + swizzled sState SMEM)."""
    assert v.is_cuda and v.dtype == torch.bfloat16 and v.is_contiguous()
    assert out.is_cuda and out.dtype == torch.bfloat16 and out.is_contiguous()
    B, T, H, K = v.shape
    assert K == D and T % CHUNK == 0
    ...
    key = (B, T, H)
    if key not in _compiled_cache_k2C:
        stream = cuda_drv.CUstream(torch.cuda.current_stream().cuda_stream)
        v_flat = v.view(T_total, H, D)
        out_flat = out.view(T_total, H, D)
        _compiled_cache_k2C[key] = cute.compile(
            run_k2_phaseC,
            from_dlpack(v_flat.detach(), assumed_align=16),
            from_dlpack(beta.detach(), assumed_align=16),
            from_dlpack(ws_qd.detach(), assumed_align=16),
