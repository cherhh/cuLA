...
THREADS_PER_CTA = 160  # 128 compute (4 MMA warps) + 32 load (1 TMA warp)
N_WARPS = 4
...
def k2_phaseL_kernel(
    ...
    # Empty mbarriers — producer (load warp) waits on these before re-issuing
    # TMA into a slot; compute warps arrive on these after consuming the slot.
    ...
    if warp_idx == 0:
        with cute.arch.elect_one():
            for s in cutlass.range_constexpr(STAGES):
                # Full barrier: 1 producer arrival per stage (TMA completion).
                cute.arch.mbarrier_init(sMbar_ptr + cutlass.Int32(s), cutlass.Int32(1))
                # Empty barrier: 1 consumer arrival per stage (compute lane 0).
    ...
    if warp_idx == 4:
        ...
        for t in cutlass.range(t_tiles, unroll=1):
            ...
            # COMPUTE-ONLY (load warp is in its own loop): named barrier id=1.
            cute.arch.barrier(barrier_id=1, number_of_threads=128)

            if warp_idx == 0:
                cute.copy(tma_atom_out, tOs[(None,)], tOg[(None, t_g, 0, head_idx)])
                cute.arch.cp_async_bulk_commit_group()
                # Signal load warp that slot s_dyn is now empty (single arrival;
                # producer expected_count=1 on each empty mbarrier).
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive(sMbarE_ptr + s_dyn)
            cute.arch.cp_async_bulk_wait_group(0, read=True)
            cute.arch.barrier(barrier_id=1, number_of_threads=128)
            # Advance rotating stage / phase.
...
def run_k2_phaseL(
    ...
    smem_bytes = (
        ...
        + (D * 4)  # sGt
        + (CHUNK * 2)  # sBeta
        + STAGES_LOCAL * 8  # full mbarriers
        + STAGES_LOCAL * 8  # empty mbarriers (warp specialization)
        + 256  # slack
    )
