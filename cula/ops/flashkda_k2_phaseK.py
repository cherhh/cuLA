...
def k2_phaseK_kernel(
    ...
    # Phase K: pad sKr_T row stride to CHUNK+8=24 to break bank-conflict pattern
    ...
    # Single rotating phase counter — expected mbarrier phase for slot
    # `s_dyn` at iteration t equals (t // STAGES) & 1, which flips after each
    # full pass over the STAGES slots. Cheaper than per-slot if/elif dispatch.
    phase = cutlass.Int32(0)
    s_dyn = cutlass.Int32(0)  # rotating stage index (0..STAGES-1)
    ...
    for t in cutlass.range(t_tiles, unroll=1):
        ...
        # Wait for TMA into stage s_dyn (single mbarrier_wait — no dispatch).
        cute.arch.mbarrier_wait(sMbar_ptr + s_dyn, phase)
        ...
        cute.arch.barrier()
        # Rotate stage index: s_dyn = (s_dyn + 1) % STAGES; flip phase on wrap.
...
def run_k2_phaseK(
    ...
    # 3-stage SMEM: 4 staged QK tiles + 2 staged CC tiles + sOut(unstaged)
    # + sState + sKr_T (padded) + sGt + sBeta + 3 mbarriers + slack.
    # NOTE: keep in sync with the kernel-side STAGES constexpr.
    STAGES_LOCAL = 3
    KR_T_PAD_LOCAL = CHUNK + 8
    smem_bytes = (
        D * D * 2  # sState
        + STAGES_LOCAL * 4 * (CHUNK * D * 2)  # sV/sKd/sQd/sKr staged
        + STAGES_LOCAL * 2 * (CHUNK * CHUNK * 2)  # sINV/sMqk staged
        + (CHUNK * D * 2)  # sOut (unstaged)
        + (D * KR_T_PAD_LOCAL * 2)  # sKr_T (padded; sU_T eliminated)
        + (D * 4)  # sGt
        + (CHUNK * 2)  # sBeta
        + STAGES_LOCAL * 8  # mbarriers
        + 256  # slack
    )
