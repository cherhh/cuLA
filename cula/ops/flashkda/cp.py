...
def _auto_s_split(device: torch.device, n_seqs: int, H: int) -> int:
    sm_count = _sm_count(device)
    # Fill 2 CTAs/SM with rerun CTAs (S_total * H of them). floor, not ceil:
    # spilling past the wave boundary adds a ragged tail wave (measured: H=64
    # auto=5 -> 320 CTAs on 264 slots is slower than s_split=4 -> 256).
    # Used together with the AUTO_MIN_SEG_TILES per-sequence clamp.
    return max(1, (2 * sm_count) // (H * n_seqs))
