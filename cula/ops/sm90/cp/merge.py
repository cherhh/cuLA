...
"""CuTeDSL merge kernel for FlashKDA intracard-CP (SM90).
...
from cula.ops.sm90.k2 import D
...
class Merge:
    ...
    def __call__(
        ...
        self.kernel(
            b_seg, m_seg, carries, init, first_ptr, nseg_ptr,
            sH_layout, sM_layout, sB_layout, tiled_copy, tiled_store,
        ).launch(
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.has_init):
            ...
            for j in cutlass.range_constexpr(self.num_col_tiles):
                gI = cute.local_tile(
                    g_init, tiler=(_BR, _BN), coord=(i_r, j)
                )
            ...
            for ri in cutlass.range_constexpr(_rows_per_thr):
                ...
                for g in cutlass.range_constexpr(_n_col_groups):
                    for c in cutlass.range_constexpr(_VEC):
                        sH[r, g * _col_stride + t_n * _VEC + c] = cutlass.Float32(
                            0.0
                        )
            cute.arch.barrier()
        ...
        for idx in cutlass.range(0, n_seg - 1, unroll=0):
            ...
            for j in cutlass.range_constexpr(self.num_col_tiles):
                gC = cute.local_tile(
                    g_c_out, tiler=(_BR, _BN), coord=(i_r, j)
                )
                sHj = cute.local_tile(sH, tiler=(_BR, _BN), coord=(0, j))
                cute.copy(
                    tiled_store,
...
def _compile_merge(H: int, has_init: int):
    ...
    b_seg_fake = make_fake_compact_tensor(
        cutlass.Float32, (sym_s, H, D, D), stride_order=(3, 2, 1, 0), assumed_align=128,
    )
    ...
    init_fake = make_fake_compact_tensor(
        cutlass.Float32, (sym_n, H, D, D), stride_order=(3, 2, 1, 0), assumed_align=128,
    )
    ...
    return cute.compile(
        kernel_obj,
        b_seg_fake, m_seg_fake, carries_fake, init_fake,
        first_fake, nseg_fake,
        cutlass.Int32(1),
        stream_fake,
    )
